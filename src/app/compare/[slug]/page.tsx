...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `${appUrl}/compare/${entry.slug}`
    },
    openGraph: {
      title: entry.title,
      description: entry.description,
      type: "article",
      url: `${appUrl}/compare/${entry.slug}`,
      ...
      <article className="rounded-2xl border border-[var(--line)] bg-[var(--card)] p-6 shadow-xl shadow-black/10 md:p-8">
        ...
        <footer className="mt-10 border-t border-[var(--line)] pt-5">
          <p className="text-sm text-[var(--text-soft)]">
            Lihat perbandingan lain di <Link href="/compare" className="font-medium text-[var(--accent)]">hub compare</Link>
            .
          </p>
        </footer>
      </article>
