...
export async function generateStaticParams() {
  return getSeoEntries("compare").map((entry) => ({ slug: entry.slug }));
}
...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const entry = getSeoEntry("compare", slug);

  if (!entry) {
    return {
      title: "Halaman Perbandingan Tidak Ditemukan | Chandra Notepad"
    };
  }
  ...
  return {
    ...
    alternates: {
      canonical: `${appUrl}/compare/${entry.slug}?lang=${language}`,
      languages: {
        id: `${appUrl}/compare/${entry.slug}?lang=id`,
        en: `${appUrl}/compare/${entry.slug}?lang=en`
      }
    },
    openGraph: {
      title: entry.title,
      description: entry.description,
      type: "article",
      url: `${appUrl}/compare/${entry.slug}?lang=${language}`,
...
export default async function CompareArticlePage({ params, searchParams }: Props) {
  const { slug } = await params;
  const { lang } = await searchParams;
  const language = lang === "en" ? "en" : "id";
  ...
  return (
    <section className="mx-auto w-full max-w-4xl px-4 py-12">
      ...
      <article className="rounded-2xl border border-[var(--line)] bg-[var(--card)] p-6 shadow-xl shadow-black/10 md:p-8">
        <ContentPageToolbar currentPath={`/compare/${entry.slug}`} language={language} />
        <p className="text-xs uppercase tracking-[0.18em] text-[var(--text-soft)]">Compare</p>
        ...
        <footer className="mt-10 border-t border-[var(--line)] pt-5">
          <p className="text-sm text-[var(--text-soft)]">
            {isEnglish ? "See more comparisons in the " : "Lihat perbandingan lain di "}
            <Link href={`/compare?lang=${language}`} className="font-medium text-[var(--accent)]">
              {isEnglish ? "comparison hub" : "hub compare"}
            </Link>
            .
          </p>
        </footer>
